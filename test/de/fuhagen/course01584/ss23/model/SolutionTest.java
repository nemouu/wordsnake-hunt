...
class SolutionTest {
	...
	class Einfache_Tests {
		...
		void testeGetSchlangeMitIDEins() {
			...
			assertEquals(bspLoesung.getSchlangeMitSchlangenartID("A0").getArt().toString(),
		...
		void testeGetSchlangeMitIDZwei() {
			...
			bspLoesung.addSchlange(schlange1);
			bspLoesung.addSchlange(schlange2);
			assertEquals(bspLoesung.getSchlangeMitSchlangenartID("A1").getArt().toString(),
		...
		void testeAddSchlange() {
			...
			bspLoesung.addSchlange(schlange1);
			assertEquals("A0", bspLoesung.getSchlangen().get(0).getArt().getId(),
					"\nDie erhaltene ID der Schlange stimmt nicht mit der tatsaechlichen ueberein.");
			assertEquals("Distanz", bspLoesung.getSchlangen().get(0).getArt().getStruktur().getArt(),
					"\nDie erhaltene Nachbarschaftsstruktur stimmt nicht mit der tatsaechlichen ueberein.");
			assertEquals("FERNUNI", bspLoesung.getSchlangen().get(0).getArt().getZeichenkette(),
					"\nDie erhaltene Zeichenkette der Schlange stimmt nicht mit der tatsaechlichen ueberein.");
			assertEquals(2, bspLoesung.getSchlangen().get(0).getArt().getPunkte(),
					"\nDie errechneten Punkte der Schlange stimmen nicht mit der tatsaechlichen ueberein.");
			assertEquals(3, bspLoesung.getSchlangen().get(0).getArt().getAnzahl(),
		...
		void testeEntferneLetzteSchlangeEins() {
			...
			assertEquals("A1", bspLoesung.getSchlangen().get(0).getArt().getId(),
					"\nDie erhaltene ID der Schlange stimmt nicht mit der tatsaechlichen ueberein.");
			assertEquals("Distanz", bspLoesung.getSchlangen().get(0).getArt().getStruktur().getArt(),
					"\nDie erhaltene Nachbarschaftsstruktur stimmt nicht mit der tatsaechlichen ueberein.");
			assertEquals("JUNITTEST", bspLoesung.getSchlangen().get(0).getArt().getZeichenkette(),
					"\nDie erhaltene Zeichenkette der Schlange stimmt nicht mit der tatsaechlichen ueberein.");
			assertEquals(2, bspLoesung.getSchlangen().get(0).getArt().getPunkte(),
					"\nDie errechneten Punkte der Schlange stimmen nicht mit der tatsaechlichen ueberein.");
			assertEquals(4, bspLoesung.getSchlangen().get(0).getArt().getAnzahl(),
					"\nDie errechnete Anzahl von Schlangen stimmt nicht mit der tatsaechlichen ueberein.");
		}
